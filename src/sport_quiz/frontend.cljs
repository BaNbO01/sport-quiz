...
(defonce last-chosen-answer (r/atom nil)) ; NOVI ATOM za pamćenje izabranog odgovora
      ...
      {:params {:session-id session-id :answer answer}
       ...
       :handler #(let [server-resp %
                   ...
                   (let [completed? (:completed? new-state-from-server)]
                     (if-not completed?
                       (js/setTimeout (fn [] ; <-- KORIGOVANO: Koristimo fn umesto ugnježdenog #()
                                        (reset! last-answer-correct? nil)
                                        (reset! correct-answer nil)
                                        (reset! last-chosen-answer nil)
  ...
  (fn []
    (when-let [q (:current-question @game-state)]
      (let [correct-ans-display @correct-answer
            chosen-ans-display @last-chosen-answer] ; UZIMAMO IZABRANI ODGOVOR IZ ATOMA
        [:div
         [progress-bar (:progress @game-state)]

         ;; ISPRAVLJENO: Uzimamo (first ...) da dobijemo samo ime fajla
         ...
         [:div
          (for [opt (:options q)]
            ...
            (let [is-correct-display (and correct-ans-display (= opt correct-ans-display))
                  ;; Oboji crveno samo ako je odgovor NETAČAN I ako je korisnik KLIKNUO BAŠ NA TAJ TASTER
              ...
              [:button {:on-click #(submit-answer opt)
                        :disabled (some? @last-answer-correct?) ; Blokiramo tastere dok se prikazuje rezultat
